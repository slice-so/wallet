...
ERC-20 checkout uses one atomic, deterministic batch: an exact aggregate approval for each checkout currency, an `ERC20AllowanceGuard.assertAllowance` for the same token, spender, and amount, then exactly one canonical `ProductsModule.buy` or `ProductsModule.pay`. The frame and co-signer reject missing, duplicate, reordered, under-, or over-approvals and assertions. Settlement also requires exact two-sided balance deltas, so reverting, false-returning, no-op, fee, burn, malformed, or mint-without-source-debit token behavior fails the complete purchase.
