...
import type { SliceKernelPolicy } from "./types/kernel"
import type {
...
const conditionCode = {
  ...
  not_equal: 5
} as const satisfies Record<WalletPolicyParameterCondition, number>

const kernelCallType = "0x00" as const
...
export const toWalletPermissionPolicies = (
  descriptor: WalletPolicyDescriptor
): readonly SliceKernelPolicy[] => {
  const normalized = normalizeWalletPolicyDescriptor(descriptor)
  const policies: SliceKernelPolicy[] = [
    {
      address: sliceWalletKernelAddresses.callPolicyV005,
  ...
  if (normalized.grantKind === "management") {
    policies.push({
      address: sliceWalletKernelAddresses.slicerRegistryPolicy,
      data: "0x",
      kind: "slicer-registry"
    })
  }

  policies.push({
    address: sliceWalletKernelAddresses.timestampPolicy,
    data: encodeAbiParameters(
      [
        { name: "validAfter", type: "uint48" },
        { name: "validUntil", type: "uint48" }
      ],
      [normalized.validAfter, normalized.validUntil]
    ),
    kind: "timestamp"
  })
  ...
  if (normalized.rateLimit !== undefined) {
    policies.push({
      address: sliceWalletKernelAddresses.rateLimitPolicy,
      data: encodePacked(
        ["uint48", "uint48", "uint48"],
        [
          normalized.rateLimit.intervalSec,
          normalized.rateLimit.count,
          normalized.validAfter
        ]
      ),
      kind: "rate-limit"
...
export const getWalletPermissionInstallConfiguration = (
  ...
  return {
    permissionFlag: "0x0002" as const,
    policies: policies.map((policy) => ({
      configuration: policy.data,
      identity: concat(["0x0000", policy.address]),
      type: policy.kind
    }))
  }
}
