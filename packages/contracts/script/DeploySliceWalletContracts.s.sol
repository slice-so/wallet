...
contract DeploySliceWalletContractsScript is Script {
    ...
    uint256 private constant WEIGHTED_P256_SIGNER = 1 << 3;
    uint256 private constant ERC6492_BOOTSTRAP_FACTORY = 1 << 4;
    uint256 private constant ALL_CONTRACTS = (1 << 5) - 1;
    ...
    bytes32 private constant ERC6492_BOOTSTRAP_FACTORY_SALT = keccak256("slice.kernel.erc6492-bootstrap-factory.v1");

    function run() external returns (address[5] memory deployed) {
        ...
        if (deploymentMask & WEIGHTED_P256_SIGNER != 0) deployed[3] = _deployWeightedP256Signer();
        if (deploymentMask & ERC6492_BOOTSTRAP_FACTORY != 0) deployed[4] = _deployErc6492BootstrapFactory();
    }
